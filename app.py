...
st.title("Pré-Avaliação Neuropsicológica")
...
with st.form(key="form_anamnese"):
    ...
    st.markdown("---")
    # === HISTÓRICO DE MEDICINA E NEUROLOGIA (OPCIONAL) ===
    st.header("Uso de Tecnologias/Neurotecnologias")
    uso_neuro = st.text_area(
        "Informa se já utiliza Neurofeedback, tDCS, Hipnose, BrainTap, Muse ou outros", height=60
    )
    ...
    if submit_button:
        ...
        doc.add_paragraph(f"Médico: {hx_medico if hx_medico else 'Nenhum registrado'}")
        doc.add_paragraph(f"Familiar: {hx_familiar if hx_familiar else 'Nenhum registrado'}")
        ...
        doc.add_page_break()
        doc.add_heading("Uso de Tecnologias/Neurotecnologias", level=2)
